...
import {
    access_levels,
    cells, cell_families, teams, column_types, files,
    cell_models, cell_manufacturers, cell_form_factors, cell_chemistries,
    equipment_types, equipment_manufacturers, equipment_models,
    schedule_identifiers, column_mappings,
    file_summary, file_applicable_mappings
...
const paginate = (results: unknown[], url: string) => {
    ...
    return {
        results: subset,
        count: subset.length,
        next: offset + limit < results.length ? `${url_base}?offset=${offset + limit}&limit=${limit}` : null,
        previous: offset - limit >= 0 ? `${url_base}?offset=${offset - limit}&limit=${limit}` : null
    }
}

const build_get_endpoints = (resource_name: keyof typeof resources): HttpResponseResolver =>
    ({request}) => {
        ...
        if (/describe\/$/.test(url_extras)) {
            ...
            for (const key in d) {
                if (['custom_properties'].includes(key))
                    continue
        ...
        const parts = url_extras.split('/').filter(x => x && /^\?/.test(x) === false)
        if (parts.length > 0) {
            if (resource_name === 'files' && parts.length > 1) {
                ...
                if (parts[0] !== files[0].id) {
                    throw new Error(`Sub-pages are only implemented for file ${files[0].id}`)
                }
                if (parts[1] === 'summary') {
                    console.log('request.url', request.url.toString(), '-> summary')
                    return HttpResponse.json(file_summary)
                }
                if (parts[1] === 'applicable_mappings') {
                    console.log('request.url', request.url.toString(), '-> applicable_mappings')
...
const build_stub_endpoints = (resource_name: keyof typeof resources): HttpResponseResolver =>
    ({request}) => {
...
export const restHandlers = [
    ...Object.keys(resources).map((r) => http.get(
        RegExp(`/${r}/`),
        build_get_endpoints(r as keyof typeof resources)
    )),
    ...Object.keys(resources).map((r) => http.patch(
        RegExp(`/${r}/`),
        build_stub_endpoints(r as keyof typeof resources)
    )),
    ...Object.keys(resources).map((r) => http.post(
        RegExp(`/${r}/`),
        build_stub_endpoints(r as keyof typeof resources)
    )),
