...
export type ClientCodeSupportedLanguage = 'python' | 'matlab' | 'julia'
        ...
        return (
            <FormControl fullWidth>
                ...
                <Select
                    ...
                    onChange={(e) =>
                        setSelectedDatasetIds(e.target.value as string[])
                    }
...
export default function ClientCodeDemo(props: { fileQueryLimit?: number }) {
    ...
    const [language, setLanguage] =
        useState<ClientCodeSupportedLanguage>('python')
    ...
    return (
        <Card>
            <CardHeader
                ...
                subheader={
                    'Copy template code to download datasets using the Galv API'
                }
            ...
            <CardContent hidden={!open}>
                <Stack spacing={1}>
                    <FormControl fullWidth>
                        <InputLabel id={`${dataset_ids.join()}-select-label`}>
                            Language
                        </InputLabel>
                        <Select
                            value={language}
                            ...
                            onChange={(e) =>
                                setLanguage(
                                    e.target
                                        .value as ClientCodeSupportedLanguage,
                                )
                            }
                        >
                            {['Python', 'Julia', 'Matlab'].map((lang) => (
                                <MenuItem value={lang.toLowerCase()} key={lang}>
                                    {lang}
                                </MenuItem>
                            ))}
                        </Select>
