...
export function is_resource_type(
    v: unknown,
): v is TypeChangerLookupKey | TypeChangerAutocompleteKey {
    if (typeof v === 'string') {
...
export function is_tvn_wrapper(
    ...
    if (v instanceof Object)
        return Object.entries(v).every(([k, v]) =>
            is_tvn(v, strict, verbose, { key: k, context }),
        )
    if (verbose)
        console.warn(`TypeValueNotationWrapper check found non-object`, v)
    return false
}
...
export function is_tvn(
    ...
    if (v instanceof Object) {
        ...
        if (
            ...
            if (
                typeof unknown._type !== 'string' ||
                unknown._type.length === 0
            ) {
                if (verbose)
                    console.warn(
            ...
            if (_v._value instanceof Array) {
                ...
                return arr._value.every((x, i) =>
                    is_tvn(x, strict, verbose, { index: i, context }),
                )
            }
            ...
            if (
                !['string', 'number', 'boolean'].includes(typeof _v._value) &&
                _v._value !== null
            ) {
                if (verbose)
                    console.warn(
    ...
    if (verbose)
        console.warn(`TypeValueNotation check found non-object`, {
            candidate: v,
            context,
        })
    return false
...
export const validate_type_value_notation = (
    v: unknown,
    allow_wrappers = true,
): TypeValueNotation | TypeValueNotationWrapper => {
    if (is_tvn(v, true) || (allow_wrappers && is_tvn_wrapper(v, true))) return v
    throw new Error(`Invalid TypeValueNotation: ${v}`)
}
export const to_type_value_notation = (
    v: Serializable,
    field_info?: Field,
): TypeValueNotation => {
    ...
    if (field_info?.type) {
        ...
        if (field_info.many && v instanceof Array)
            return {
                _type: 'array',
                _value: (v as Serializable[]).map((x) =>
                    to_type_value_notation(x, { ...field_info, many: false }),
                ),
            }
        // Objects resume automatic type detection
        if (field_info.type === 'object') return to_type_value_notation(v)
        // Other types use the field_info.type property to determine the type of the value
        return {
            _type: field_info.type,
            _value: v as TypeValueNotation['_value'],
        }
    }
    ...
    if (v instanceof Object) {
        return {
            _type: 'object',
            _value: Object.fromEntries(
                Object.entries(v).map(([k, v]) => [
                    k,
                    to_type_value_notation(v),
                ]),
            ),
        }
    }
...
export const to_type_value_notation_wrapper = (
    ...
    if (lookup_key) {
        ...
        return Object.fromEntries(
            Object.entries(v).map(([k, v]) => [
                k,
                to_type_value_notation(v, fields[k as keyof typeof fields]),
            ]),
        )
    }
    return Object.fromEntries(
        Object.entries(v).map(([k, v]) => [k, to_type_value_notation(v)]),
    )
}
export const from_type_value_notation = (
    v: TypeValueNotation | TypeValueNotationWrapper,
): typeof v extends TypeValueNotationWrapper
    ? SerializableObject
    : Serializable => {
    ...
    if (!wrapper && !is_tvn(v)) {
        ...
        throw new Error(
            `from_type_value_notation: input is not in TypeValue notation`,
        )
    }
    if (wrapper)
        return Object.fromEntries(
            Object.entries(v).map(([k, v]) => [k, from_type_value_notation(v)]),
        )
    if (v._type === 'object' && v._value instanceof Object)
        return Object.fromEntries(
            Object.entries(v._value).map(([k, v]) => [
                k,
                from_type_value_notation(v),
            ]),
        )
