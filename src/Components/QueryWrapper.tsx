...
export type QueryDependentElement = (
...
export default function QueryWrapper(props: QueryWrapperProps) {
    const loading = props.queries.some(
    ...
    if (error)
        return typeof props.error === 'function'
            ? props.error(props.queries.filter((query) => query.isError))
            : props.error
    if (loading) return props.loading
    if (success)
        return typeof props.success === 'function'
            ? props.success(props.queries.filter((query) => query.isSuccess))
            : props.success
    return (
