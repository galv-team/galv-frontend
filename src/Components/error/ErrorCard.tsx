...
export type ErrorCardProps = ErrorProps & {
    header?: ReactElement<CardHeaderProps>
}

export default function ErrorCard(props: ErrorCardProps & CardProps) {
    const { classes } = useStyles()
    let content = <CardContent>{props.message}</CardContent>
    if (!props.message) {
        if (props.status === 401)
            content = (
                <CardContent>
                    You must <Link to={'/login'}>log in</Link> to see this item.
                </CardContent>
            )
        ...
        else content = <CardContent>An unknown error occurred.</CardContent>
    }
    return (
        <Card
            className={clsx(classes.itemCard, classes.error)}
            {...(props as CardProps)}
        >
