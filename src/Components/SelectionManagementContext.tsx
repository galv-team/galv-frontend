...
const SelectionManagementContext = createContext(
    {} as ISelectionManagementContext,
)
...
export const useSelectionManagement = () => {
    ...
    if (context === undefined) {
        throw new Error(
            'useSelectionManagement must be used within a SelectionManagementContextProvider',
        )
    }
    return context
}

export default function SelectionManagementContextProvider({
    children,
}: PropsWithChildren) {
    const [selected, setSelected] = useState<string[]>([])

    const to_url = (resource: Selectable) =>
    ...
    const isSelected = (resource: Selectable) =>
        selected.includes(to_url(resource))
    const toggleSelected = (resource: Selectable) =>
        isSelected(resource) ? deselect(resource) : select(resource)
    ...
    return (
        <SelectionManagementContext.Provider
            value={{
                ...
                select,
                deselect,
                setSelected: (resource: Selectable, selected: boolean) =>
                    selected ? select(resource) : deselect(resource),
                toggleSelected,
                isSelected,
