...
import {FAMILY_LOOKUP_KEYS, FIELDS, GalvResource, get_has_family, LookupKey} from "../constants";
    ...
    (data: AxiosResponse<GalvResource>) => {
        Object.entries(FIELDS[lookup_key]).forEach(([k, v]) => {
            if (v.transformation !== undefined)
                data.data[k as keyof typeof data.data] = v.transformation(data.data[k as keyof typeof data.data])
        })
...
function ApiResourceContextWithFamilyProvider<T extends GalvResource>(
    ...
    const family_query = useRetrieveQuery(
        family_lookup_key,
        has(query.data?.data, 'family')? id_from_ref_props(query.data.data.family) : "never",
        {extra_query_options: {
